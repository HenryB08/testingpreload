...
// Intro: the letters of "syntrexio.com" slide in and settle (a small text move),
// pause, then a blue ring draws around the word and fills inward to reveal the
// Syntrex glass orb on top of it. Hands off to the preloader via onComplete().
export default function Intro({ onComplete }) {
  ...
  useEffect(() => {
    ...
    // 2. brief pause with the words at rest, before the circle starts
    tl.to({}, { duration: 0.3 })
    // 3. blue ring draws around the word
    tl.to(ring, { strokeDashoffset: 0, duration: 0.65, ease: 'power2.inOut' })
    // 4. fill inward to reveal the orb (outer edge fixed, stroke thickens inward)
    tl.to(
      fill,
